...
% Krista Longnecker; 24 March 2026, updated 25 March 2026
%
...
%% now work with the older files 
% These files were calibrated in 2020. Put the results into the CTDdatadir
% so we can combine everything into one big file
insideFolder = 'Calibrated CTD_2020.1.21'; %inside each folder will be a folder with this name

%go find the folder names from the older files
cd(CTDdatadir_older)
D = dir();
dirlist = D(3:end); %this skips over the directories . and ..
clear D 

%copy over the data file into the folder with the newer cruises
for a = 1:length(dirlist)
    thisdir = dirlist(a).name;
    cd(fullfile(thisdir,insideFolder))
    temp = dir('*_ctd.txt');
    copyfile(temp.name,fullfile(CTDdatadir,temp.name(2:end)))
    cd(CTDdatadir_older)
    clear thisdir temp
end
clear a
...
%%  now load and label CTD data, including derived variables
cd(CTDdatadir);
