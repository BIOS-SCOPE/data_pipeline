...
% up through row ~55 in this code. There should be no need to change
% anything past that point.
% (2) The output from this code will be a CSV file that will be saved in
% the place defined at line 29
% (3) This script calculates the derived variables (season, sunrise/sunset,
% vertical zones, MLD depths) for all rows in the BIOS-SCOPE COMBINED
% MASTER file
%
...
% Krista Longnecker, updated 24 August 2026 --> removed columns from COMBINED
...
CTDdatadir = fullfile(rootdir,'RawData\processedCTDdata'); %this is where the processed CTD files are
...
for icru = 1:ncru
    ...
    if isempty(dirlist)
        ...
        keyboard %stop here while I am troubleshooting
        continue
    end
    fname = dirlist.name;
    disp(['Loading ',fname])
