...
% Krista Longnecker, updated 24 August 2026 --> trimmed down COMBINED
% MASTER file and update the seasons
...
CTDdatadir = fullfile(datadir,'BCODMOdataset3918_BATS_v11'); %put CTD data here...KEEP name so we know which CTD release we are adding
CTDprocessedDir = fullfile(datadir,'/processedBCODMOdata') %keep separate from files from BATS all time...though they should be the same
...
%%%%%%%%%%%%%%%% Krista Longnecker, updated 24 August 2026
%%%%%%%%%%%%%%%%
do_plots = 0; %set this to 1 if you want plots - unlikely for a large number of cruises

%make sure the CTDprocessedDir is empty before starting...
delete(fullfile(CTDprocessedDir,'*.*'))
...
for ii = 1:length(uniqueCruises)    
   ...
   CTD = create_BIOSSCOPE_ctd_files(oneCruise,season_dates,do_plots,CTDprocessedDir); %update KL 6/12/2026
...
nfiles = length(dirlist);
    ...
    for ii = 1:nfiles
       ...
       do_plots = 0;
       showOutput = 0; %adding 8/24/2026
       CTD = create_BIOSSCOPE_ctd_files(infile,MAXZ,season_dates,do_plots,showOutput);
